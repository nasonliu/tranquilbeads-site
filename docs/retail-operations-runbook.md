...
从 Preview 环境删除此变量并重新部署，使新 deployment 不再接受短期凭证。
...
5. Configure transactional and campaign delivery with `RETAIL_RESEND_API_KEY`, `RETAIL_EMAIL_FROM`, and a server-only `RETAIL_PORTAL_TOKEN_SECRET` that is at least 32 characters. Set `RETAIL_EMAIL_REPLY_TO` to the monitored Google Workspace support mailbox; Resend remains the automated sender while replies are handled in Workspace. Set the truthful business mailing address in `RETAIL_MARKETING_POSTAL_ADDRESS` before scheduling a promotion. Promotional campaigns only target active double-opt-in subscribers and always include that address plus an unsubscribe link. The checkout consent is optional and unchecked. Never expose these secrets to the browser. Rotation does not automatically invalidate database-backed confirmation links: existing links remain valid until expiry or explicit revocation. Before rotation, drain or manually handle pending/failed confirmation notifications; retrying the same notification after the secret changes can conflict with its stored token hash. Rotate only in a controlled support window, then manually reissue replacement links where required.
6. Configure YunExpress only with server-side `YUNEXPRESS_APP_ID`, `YUNEXPRESS_APP_SECRET`, `YUNEXPRESS_SOURCE_KEY`, `YUNEXPRESS_ENV` (`sandbox` or `production`), and optional `YUNEXPRESS_ORIGIN_CODE`. Enable customer quotes with `RETAIL_DYNAMIC_SHIPPING_ENABLED=true`; use a dedicated 32+ character `RETAIL_SHIPPING_QUOTE_SECRET` when possible. `RETAIL_SHIPPING_BUFFER_BPS` defaults to 1000 (10%), the quote is valid for 15 minutes, and `RETAIL_FREE_SHIPPING_THRESHOLD_USD_MINOR` defaults to 9900. The cart aggregates per-SKU weight and dimensions plus packaging before each read-only rate request, converts a fresh CNY quote into USD, and revalidates the signed quote before PayPal order creation. Production payment creation fails closed unless YunExpress is in production mode and the shipping/email gates are complete. Provider order creation remains disabled until the create/cancel/label/tracking workflow passes UAT; launch fulfilment therefore remains an audited manual tracking-number workflow.
7. Configure the PayPal webhook URL as `/api/retail/webhook` and subscribe to checkout approval, capture completed/denied/reversed, and refund events.
8. Merge the workflow to the default branch, add the GitHub repository secrets `RETAIL_CRON_ENDPOINT=https://your-production-domain/api/cron/retail/reservations` and `CRON_SECRET` (matching Vercel), manually dispatch `Retail operations cron`, and confirm a successful response before relying on the schedule.
9. Set `RETAIL_SHOP_ENABLED=true` only after the sandbox acceptance checklist passes. Use `RETAIL_PAYMENT_MODE=sandbox` with the Sandbox endpoint in local and Preview. Live mode is permitted only in Production with `RETAIL_PAYMENT_MODE=live` and the exact Live endpoint; Preview cannot enable Live mode.
