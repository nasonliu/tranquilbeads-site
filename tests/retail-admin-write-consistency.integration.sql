...
BEGIN
  ...
  INSERT INTO retail_orders(paypal_order_id,client_request_id,currency,amount_minor,subtotal_minor,status,capture_id,captured_at,items_snapshot)
    VALUES('INTEGRATION-ORDER','10000000-0000-4000-8000-000000000009','USD',100,100,'captured','INTEGRATION-CAPTURE',now(),'[]') RETURNING id INTO order_id;
  PERFORM retail_fulfil_order(order_id,'Carrier','TRACK','Note','10000000-0000-4000-8000-000000000010');
  PERFORM retail_fulfil_order(order_id,'Carrier','TRACK','Note','10000000-0000-4000-8000-000000000010');
  ...
  INSERT INTO retail_payment_ledger(order_id,kind,amount_minor,currency,paypal_reference,idempotency_key)
    VALUES(order_id,'payment',100,'USD','INTEGRATION-LEDGER','10000000-0000-4000-8000-000000000014') RETURNING id INTO ledger_id;
  ...
  INSERT INTO retail_orders(paypal_order_id,client_request_id,currency,amount_minor,subtotal_minor,status,items_snapshot)
    VALUES('INTEGRATION-CANCEL','10000000-0000-4000-8000-000000000016','USD',100,100,'created','[]') RETURNING id INTO cancel_order_id;
