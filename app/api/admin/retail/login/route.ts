import { z } from "zod";
import { assertSameOrigin, authenticateRetailAdminWithStoredPassword, consumeRetailAdminLoginFailure, setRetailAdminSession } from "@/src/lib/retail/admin-auth";
...
export async function POST(request: Request) {
  try {
    ...
    const actor = await authenticateRetailAdminWithStoredPassword(password, actorId);
    if (!actor) {
      try {
    ...
    const unavailable = error instanceof Error && (error.message.includes("database") || error.message.includes("credential"));
    return Response.json({ ok: false, ...(unavailable ? { error: "login_unavailable" } : {}) }, { status: unavailable ? 503 : 400, headers: { "cache-control": "no-store" } });
  }
}
