...
export async function POST(request:Request){try{await requireRetailPermission("shipping:write");await assertSameOrigin();return Response.json({ok:true,zone:await upsertShippingZone(shippingZoneDto.parse(await request.json()))},{headers:noStore});}catch(error){return fail(safeError(error),statusFor(error));}}
export async function DELETE(request:Request){try{await requireRetailPermission("shipping:write");await assertSameOrigin();await disableShippingZone(shippingDisableDto.parse(await request.json()));return Response.json({ok:true},{headers:noStore});}catch(error){return fail(safeError(error),statusFor(error));}}
