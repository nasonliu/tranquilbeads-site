...
export async function POST(request:Request,context:Context){try{const actor=await requireRetailPermission("finance:write");await assertSameOrigin();const {id}=await context.params;await closePayPalSettlementException(z.string().uuid().parse(id),settlementCloseDto.parse(await request.json()),actor);return Response.json({ok:true},{headers:noStore});}catch(error){const message=error instanceof Error?error.message:"invalid_request";const status=message==="unauthorized"?401:message==="forbidden"?403:400;return Response.json({ok:false,error:status===401?"unauthorized":status===403?"forbidden":"invalid_request"},{status,headers:noStore})}}
