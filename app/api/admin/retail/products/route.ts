...
export async function POST(request:Request){try{await requireRetailPermission("products:write");await assertSameOrigin();return Response.json({ok:true,product:await createAdminProduct(productDto.parse(await request.json()))},{status:201,headers:{"cache-control":"no-store"}})}catch(e){const message=e instanceof Error?e.message:"invalid_request";return fail(new Error(message==="unauthorized"||message==="forbidden"?message:"invalid_request"),message==="unauthorized"?401:message==="forbidden"?403:400)}}
