...
export async function DELETE(request:Request,context:ParamsContext){try{await requireRetailAdmin();await assertSameOrigin();const {id}=await context.params;const body=z.object({idempotencyKey:z.string().uuid()}).strict().parse(await request.json());return Response.json({ok:true,product:await archiveAdminProduct(z.string().uuid().parse(id),body.idempotencyKey)},{headers:{"cache-control":"no-store"}})}catch(e){return fail(e)}}
