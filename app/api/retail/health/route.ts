...
export async function GET(){
  ...
  try{
    ...
    const ready=rows[0]?.checkout_ready===true&&rows[0]?.shipping_ready===true&&Number(rows[0]?.active_shipping_zones??0)>0;
    return Response.json({ok:ready,status:ready?"ready":"configuration_required",database:true,paymentConfigured:true,activeShippingZones:Number(rows[0]?.active_shipping_zones??0),notificationsConfigured:Boolean(process.env.RETAIL_RESEND_API_KEY&&process.env.RETAIL_EMAIL_FROM),blobConfigured:Boolean(process.env.BLOB_READ_WRITE_TOKEN&&process.env.RETAIL_BLOB_HOSTNAME)},{status:ready?200:503,headers:noStore});
