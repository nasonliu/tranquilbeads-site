...
function copy(locale: "en" | "ar" | "zh") {
  if (locale === "ar") return {
    title: "تفاصيل الطلب", reference: "مرجع الطلب", payment: "حالة الدفع", total: "الإجمالي", ordered: "تاريخ الطلب", items: "المنتجات", shipping: "التسليم", carrier: "شركة الشحن", tracking: "رقم التتبع", unavailable: "هذه الصفحة غير متاحة.",
  ...
  if (locale === "zh") return {
    title: "订单详情", reference: "订单编号", payment: "付款状态", total: "订单合计", ordered: "下单时间", items: "商品", shipping: "配送", carrier: "承运商", tracking: "物流单号", unavailable: "此页面暂不可用。",
    paid: "已付款", pending: "处理中", refunded: "已退款", failed: "付款失败", fulfilled: "已发货", unfulfilled: "备货中", none: "暂未提供物流信息。", returns: "退货",
  };
  return {
    title: "Order details", reference: "Order reference", payment: "Payment status", total: "Order total", ordered: "Ordered", items: "Items", shipping: "Delivery", carrier: "Carrier", tracking: "Tracking number", unavailable: "This page is unavailable.",
...
export default async function CustomerPortalPage({ params }: CustomerPortalPageProps) {
  ...
  return <main className="noor-container pt-12">
    <section className="noor-panel mx-auto max-w-2xl rounded-[1.75rem] p-7">
      ...
      <div className="mt-7 border-t border-black/10 pt-5"><h2 className="text-lg font-semibold">{labels.items}</h2><ul className="mt-3 space-y-2 text-sm">{order.items.map((item, index) => <li key={`${productTitle(item, locale)}-${index}`} className="flex justify-between gap-4"><span>{productTitle(item, locale)}</span><span>×{Number(item.quantity ?? 0)}</span></li>)}</ul></div>
