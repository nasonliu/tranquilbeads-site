...
import { listStorefrontProducts } from "@/src/lib/retail/operations";
...
export default async function ShopPage({ params }: PageProps<"/[locale]/shop">) {
  ...
  return <div className="space-y-12 pt-8 md:space-y-16"><PageHero eyebrow={copy.eyebrow} title={copy.title} description={copy.description} /><RetailShop locale={locale} products={products} paypalClientId={config.enabled ? config.paypalClientId : undefined} currency="USD" enabled={enabled} copy={copy} /></div>;
}
