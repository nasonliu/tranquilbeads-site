...
1. Provision Neon managed Postgres through the Vercel Marketplace and run `npm run migrate:retail`. It applies all three versioned retail migrations in order and records checksums.
2. In Vercel Project Settings, set `RETAIL_SHOP_ENABLED=true`, `DATABASE_URL`, the PayPal values listed in `.env.local.example`, and the retail public Blob-store values `RETAIL_BLOB_STORE_ID` plus the exact `RETAIL_BLOB_HOSTNAME`. Authentication may use the store's dedicated `RETAIL_BLOB_READ_WRITE_TOKEN` or Vercel runtime OIDC. Keep all secrets server-only; never prefix them with `NEXT_PUBLIC_`. Do not reuse `BLOB_READ_WRITE_TOKEN`: it belongs to the separate private outreach store. Vercel may also inject `RETAIL_BLOB_WEBHOOK_PUBLIC_KEY`, but this application does not consume it unless a Blob webhook route is added later.
