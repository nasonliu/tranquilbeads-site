...
5. Set a high-entropy `CRON_SECRET`; `vercel.json` runs authenticated capture reconciliation, reservation cleanup, and notification delivery every five minutes. This schedule requires Vercel Pro (or an equivalent authenticated scheduler). Checkout also performs safe cleanup inside its transaction. PayPal capture accepts only active stock holds: once a hold is released or expired, it cannot consume stock and must be recreated with a new checkout.
