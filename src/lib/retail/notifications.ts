...
import { customerPortalUrl, issueCustomerPortalToken } from "./customer-portal";

type Fetcher=typeof fetch;
type NotificationRow={id:string;kind:string;recipient:string;payload:Record<string,unknown>;order_id:number;public_id:string;client_request_id:string;currency:string;amount_minor:number;carrier:string|null;tracking_number:string|null};
...
export async function deliverRetailNotifications(fetcher:Fetcher=fetch){
  ...
  for(const row of rows){
    ...
    try{
      // The random bearer credential exists only while constructing this message.
      // It is neither written to the outbox nor included in error/log text.
      const portalLink=row.kind==="order_confirmed"?customerPortalUrl((await issueCustomerPortalToken(Number(row.order_id))).token):null;
      const portalHtml=portalLink?`<p><a href="${escapeHtml(portalLink)}">View your order and delivery updates</a></p>`:"";
      const html=`<h1>${escapeHtml(subject)}</h1><p>${escapeHtml(detail).replace("&lt;br&gt;","<br>")}</p><p>Order reference: ${escapeHtml(reference)}</p>${portalHtml}<p>If you need help, reply to this email.</p>`;
