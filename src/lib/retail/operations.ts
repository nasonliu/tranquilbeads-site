...
export const shippingZoneDto = z.object({ country:z.string().trim().toUpperCase().regex(/^[A-Z]{2}$/), nameEn:z.string().trim().min(1).max(100), nameAr:z.string().trim().min(1).max(100), shippingMinor:z.number().int().nonnegative(), freeShippingThresholdMinor:z.number().int().positive().nullable(), taxRateBps:z.number().int().min(0).max(10000), active:z.boolean(), idempotencyKey:z.string().uuid() }).strict();
...
export async function createAdminProduct(d: z.infer<typeof productDto>) {
  ...
  const q = sql();
  const rows = await q`SELECT * FROM retail_create_admin_product(${d.sku},${d.slug},${d.titleEn},${d.titleAr},${d.descriptionEn},${d.descriptionAr},${d.status},${d.amountMinor},${d.idempotencyKey}::uuid)`;
...
export async function updateAdminProduct(id: string, d: z.infer<typeof productUpdateDto>) {
  const q = sql(); const rows = await q`SELECT * FROM retail_update_admin_product(${id}::uuid,${d.slug ?? null}::text,${d.titleEn ?? null}::text,${d.titleAr ?? null}::text,${d.descriptionEn ?? null}::text,${d.descriptionAr ?? null}::text,${d.status ?? null}::text,${d.idempotencyKey}::uuid)`;
...
export async function changeProductPrice(id: string, d: z.infer<typeof priceDto>) { const q = sql(); const rows = await q`SELECT retail_change_product_price_with_audit(${id}::uuid,${d.amountMinor},${d.idempotencyKey}::uuid,${d.reason}) AS changed`; return rows[0]; }
...
export async function adjustInventory(d: z.infer<typeof inventoryAdjustmentDto>) { const q = sql(); const rows = await q`SELECT retail_adjust_inventory_with_audit(${d.productId}::uuid,${d.delta},${d.reason},${d.idempotencyKey}::uuid) AS adjusted`; if (!rows[0]) throw new Error("product_not_found"); }
...
export async function fulfilAdminOrder(d: z.infer<typeof fulfilmentDto>) { const q = sql(); await q`SELECT retail_fulfil_order(${d.orderId},${d.carrier},${d.tracking},${d.note},${d.idempotencyKey}::uuid)`; }
...
export async function upsertShippingZone(d:z.infer<typeof shippingZoneDto>){const q=sql();const rows=await q`SELECT * FROM retail_upsert_admin_shipping_zone(${d.country},${d.nameEn},${d.nameAr},${d.shippingMinor},${d.freeShippingThresholdMinor},${d.taxRateBps},${d.active},${d.idempotencyKey}::uuid)`;if(!rows[0])throw new Error("shipping_zone_unavailable");return rows[0];}
export async function disableShippingZone(d:z.infer<typeof shippingDisableDto>){const q=sql();const rows=await q`SELECT retail_disable_admin_shipping_zone(${d.country},${d.idempotencyKey}::uuid) AS disabled`;if(!rows[0]?.disabled)throw new Error("shipping_zone_not_found");}

export async function listCustomers() { const q = sql(); return q`SELECT c.public_id,CASE WHEN c.email IS NULL THEN NULL ELSE left(c.email,1)||'***@'||split_part(c.email,'@',2) END AS email,CASE WHEN c.name IS NULL THEN NULL ELSE left(c.name,1)||'***' END AS name,c.created_at,COALESCE(json_agg(json_build_object('id',a.id,'city',a.city,'country',a.country,'is_default',a.is_default,'archived_at',a.archived_at) ORDER BY a.created_at) FILTER(WHERE a.id IS NOT NULL),'[]') addresses FROM retail_customers c LEFT JOIN retail_addresses a ON a.customer_id=c.id GROUP BY c.id ORDER BY c.created_at DESC LIMIT 250`; }
export async function updateCustomer(id: string, d: z.infer<typeof customerUpdateDto>) {
  ...
  const rows = await q`SELECT * FROM retail_update_admin_customer(${id}::uuid,${d.name ?? null}::text,${d.addressId ?? null}::uuid,${d.recipient ?? null}::text,${d.line1 ?? null}::text,${d.line2 ?? null}::text,${d.city ?? null}::text,${d.region ?? null}::text,${d.postalCode ?? null}::text,${d.country ?? null}::text,${d.phone ?? null}::text,${d.isDefault ?? null}::boolean,${d.archive ?? null}::boolean,${hasAddress},${d.idempotencyKey}::uuid)`;
...
export async function updateReconciliation(id: string, d: z.infer<typeof reconciliationDto>) { const q = sql(); await q`SELECT retail_reconcile_with_audit(${id}::uuid,${d.status},${d.note},${d.idempotencyKey}::uuid)`; }
...
export async function attachRetailProductImage(productId: string, image: { url: string; key: string; mime: string; bytes: number; sha256: string; altEn: string; altAr: string; idempotencyKey: string }) {
  const q = sql();
  const rows = await q`SELECT * FROM retail_attach_product_image_idempotent(${productId}::uuid,${image.url},${image.key},${image.mime},${image.bytes},${image.sha256},${image.altEn},${image.altAr},${image.idempotencyKey}::uuid)`;
  return rows[0];
}
