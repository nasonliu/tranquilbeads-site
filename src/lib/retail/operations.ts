...
export async function adjustInventory(d: z.infer<typeof inventoryAdjustmentDto>) { const q = sql(); await q`SELECT retail_adjust_inventory(${d.productId}::uuid,${d.delta},${d.reason},${d.idempotencyKey}::uuid)`; await audit("inventory.adjust", "product", d.productId, d); }
