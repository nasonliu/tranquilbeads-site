...
export async function upsertShippingZone(d:z.infer<typeof shippingZoneDto>,actor: RetailAdminActor){const q=sql();const rows=await q`SELECT * FROM retail_upsert_admin_shipping_zone_as_actor(${d.country},${d.nameEn},${d.nameAr},${d.nameZh || null},${d.nameZh !== undefined},${d.shippingMinor},${d.freeShippingThresholdMinor},${d.taxRateBps},${d.active},${d.idempotencyKey}::uuid,${actor.id},${actor.name},${actor.role},${actor.legacy})`;if(!rows[0])throw new Error("shipping_zone_unavailable");return rows[0];}
