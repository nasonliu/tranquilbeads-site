...
type Props = { locale: Locale; products: RetailProduct[]; zones: RetailShippingZone[]; paypalClientId?: string; currency?: "USD"; enabled: boolean; copy: Copy };
...
export function RetailShop({ locale, products, zones, paypalClientId, currency, enabled, copy }: Props) {
  ...
  useEffect(() => { if (hydrated) try { window.localStorage.setItem(cartStorageKey, JSON.stringify(cart)); } catch { /* cart storage is optional */ } }, [cart, hydrated]);
  ...
  return <section className="noor-container grid gap-8 lg:grid-cols-[1fr_25rem]">
    <div className="grid gap-5 sm:grid-cols-2">{products.filter((product) => variantsFor(product).some((variant) => variant.available)).map((product) => {
      ...
      return <article key={product.sku} className="noor-panel rounded-[1.5rem] p-5"><Link href={`/${locale}/shop/${encodeURIComponent(product.slug ?? product.sku)}`} aria-label={`${localized(locale, product.name)} details`} className="block"><Image src={product.image} alt={localized(locale, product.name)} width={640} height={640} className="aspect-square w-full rounded-xl object-cover" /><h2 className="mt-4 text-xl font-semibold">{localized(locale, product.name)}</h2><p className="mt-2 text-sm text-muted">{localized(locale, product.description)}</p></Link>
        <p className="mt-3 text-sm font-semibold"><RetailReferenceMoney usdMinor={display.priceMinor} locale={locale} /></p><p className="mt-1 text-xs text-muted">{display.stock} {display.available ? copy.available : labels.outOfStock}</p>
    ...
    <aside className="noor-panel h-fit rounded-[1.5rem] p-6"><h2 className="text-xl font-semibold">{copy.cart}</h2>{items.length === 0 ? <p className="mt-3 text-sm text-muted">{copy.emptyCart}</p> : <>
      ...
      <button type="button" onClick={requestQuote} className="mt-5 w-full rounded-full border border-accent px-4 py-2 text-sm font-semibold text-accent">{copy.quote}</button>{quote && <QuoteBreakdown quote={quote} copy={copy} discountLabel={labels.discount} appliedLabel={labels.applied} locale={locale} />}{quote && <><p className="mt-4 rounded-xl bg-black/[.035] px-3 py-2 text-xs leading-5 text-muted">{copy.paypalUsdNotice}</p><div ref={containerRef} className="mt-5" aria-label={copy.checkout} /></>}
...
function QuoteBreakdown({ quote, copy, discountLabel, appliedLabel, locale }: { quote: RetailQuote; copy: Copy; discountLabel: string; appliedLabel: string; locale: Locale }) {
  return <dl className="mt-5 space-y-2 border-y border-black/10 py-4 text-sm"><div className="flex justify-between"><dt>{copy.subtotal}</dt><dd><RetailReferenceMoney usdMinor={quote.subtotalMinor} locale={locale} settlementFirst /></dd></div><div className="flex justify-between"><dt>{copy.shipping}</dt><dd><RetailReferenceMoney usdMinor={quote.shippingMinor} locale={locale} settlementFirst /></dd></div>{Boolean(quote.discountMinor) && <div className="flex justify-between"><dt>{discountLabel}{quote.promotionCode ? ` (${appliedLabel}: ${quote.promotionCode})` : ""}</dt><dd><RetailReferenceMoney usdMinor={-quote.discountMinor!} locale={locale} settlementFirst /></dd></div>}<div className="flex justify-between"><dt>{copy.tax}</dt><dd><RetailReferenceMoney usdMinor={quote.taxMinor} locale={locale} settlementFirst /></dd></div><div className="flex justify-between font-semibold"><dt>{copy.total}</dt><dd><RetailReferenceMoney usdMinor={quote.totalMinor} locale={locale} settlementFirst /></dd></div></dl>;
}
