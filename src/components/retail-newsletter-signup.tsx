...
export function RetailNewsletterSignup({ locale }: { locale: Locale }) {
  ...
  return <section className="noor-container pb-4" dir={ar ? "rtl" : undefined}>
    <div className="overflow-hidden rounded-[1.75rem] bg-[#1f1a15] px-6 py-8 text-[#f7efe4] sm:px-9">
      <div className="grid items-end gap-6 md:grid-cols-[1fr_minmax(18rem,28rem)]"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-[#c9d1ac]">{ar ? "عروض TranquilBeads" : "TranquilBeads offers"}</p><h2 className="mt-3 font-serif text-3xl">{ar ? "كن أول من يعرف عن المنتجات والعروض الجديدة" : "New products and thoughtful offers, by email"}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#d8cdbf]">{ar ? "اشترك باختيارك. يمكنك إلغاء الاشتراك في أي وقت، ولن يؤثر ذلك في رسائل الطلب أو الشحن." : "Opt in for occasional retail offers. You can unsubscribe at any time; order and shipping email will continue separately."}</p></div>
        <form className="flex flex-col gap-3 sm:flex-row" onSubmit={submit}><label className="sr-only" htmlFor="retail-newsletter-email">{ar ? "البريد الإلكتروني" : "Email address"}</label><input id="retail-newsletter-email" required type="email" autoComplete="email" maxLength={320} value={email} onChange={(event) => setEmail(event.target.value)} className="min-w-0 flex-1 rounded-xl border border-white/20 bg-white px-4 py-3 text-[#211b16] outline-none" placeholder={ar ? "البريد الإلكتروني" : "Email address"}/><button disabled={state === "sending"} className="rounded-xl bg-[#9cae68] px-5 py-3 font-semibold text-[#18200d] disabled:opacity-60">{state === "sending" ? (ar ? "جارٍ الاشتراك…" : "Joining…") : (ar ? "اشترك" : "Join the list")}</button></form>
      </div>{state === "done" && <p className="mt-4 text-sm text-[#c9d1ac]" role="status">{ar ? "تحقق من بريدك الإلكتروني لتأكيد الاشتراك." : "Check your email to confirm your subscription."}</p>}{state === "error" && <p className="mt-4 text-sm text-red-300" role="alert">{ar ? "تعذر الاشتراك الآن. حاول مرة أخرى." : "We could not subscribe you. Please try again."}</p>}
    </div>
  </section>;
}
